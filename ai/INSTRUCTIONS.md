...
This scraper extracts job listings for **DIRECT ONE SA** (CIF `22913844`)
from the group's applytojob board, filtered by the `DIRECT ONE` department,
and publishes them to peviitor.ro via the v1 API (`https://api.peviitor.ro/v1`).

Target: `https://electrogrup.applytojob.com/apply/jobs/?department=DIRECT ONE`
...
1. **Start with brand** — `DIRECT ONE`
2. **Search in ANAF/CUIScan** — find company by CIF `22913844`
...
7. **Scrape new jobs** — parse the DIRECT ONE applytojob board (department filter)
...
> (ANOFM or any other scraper). Stale
...
4. **Scrape jobs** — parse the DIRECT ONE applytojob board
...
company.py / anaf.py (validate company)
    ...
    │
    ▼ (if active)
scrape applytojob board (?department=DIRECT ONE)
    │
    ▼
...
- **Applytojob board**: `https://electrogrup.applytojob.com/apply/jobs/?department=DIRECT ONE` — listing HTML
...
python3 -m scraper.api 22913844

# Validate job URLs from peviitor by CIF (head/content/browser)
python3 -m scraper.validate_jobs 22913844 --mode head
python3 -m scraper.validate_jobs 22913844 --mode content --dry-run
python3 -m scraper.validate_jobs 22913844 --mode content --delete
```
