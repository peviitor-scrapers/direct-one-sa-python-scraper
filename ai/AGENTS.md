...
Python scraper that reads job listings from the DIRECT ONE SA applytojob
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/direct-one-sa-python-scraper --json status -q .status
```
...
(`gh issue list --repo peviitor-scrapers/direct-one-sa-python-scraper --state open`),
...
python3 -m scraper.validate_jobs 22913844 --mode head  # validate job URLs
```
